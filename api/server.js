...
app.route("/product/:uid").get(function (req, res) {
  api(req, res).then(function (api) {
    ...
    api.query(prismic.Predicates.at("document.type", "layout")).then(function (layoutContent) {
      api.getByUID("product", uid).then(function (productContent) {
        var relatedProducts, relatedArray, relatedIDs;
...
app.route("/collections").get(function (req, res) {
  api(req, res).then(function (api) {
    ...
    api.query(prismic.Predicates.at("document.type", "collection-page")).then(function (collectionPage) {
      ...
      sections.forEach(function (section) {
        ...
        api.getByUID("collection", uid).then(function (collectionSection) {
          ...
          api
            .query([prismic.Predicates.at("document.type", "product"), prismic.Predicates.at("document.tags", collectionTags)], { pageSize: 100, orderings: "[my.product.name]" })
...
app.route("/").get(function (req, res) {
  api(req, res).then(function (api) {
    api.query(prismic.Predicates.at("document.type", "layout")).then(function (layoutContent) {
      api.query(prismic.Predicates.at("document.type", "home-page")).then((pageContent) => {
